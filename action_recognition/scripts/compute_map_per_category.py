...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate action recognition results."
    )
    ...
    parser.add_argument(
        ...
        choices=[0, 1, 2],
        default=1,
        help="Run configuration: 0 = visual only, 1 = pose only, 2 = visual + pose."
    )
    ...
    parser.add_argument(
        ...
        choices=[0, 1, 2, 3],
        default=0,
        help="Pose space: 0 = 3D-AA, 1 = 3D-KP, 2 = 3D-Vert, 3 = 2D-KP."
    )
...
def main():
    ...
    for SA, GI, UP, AF, UV, WO in all_runs:
        ...
        print("------------------------------------------")
        print(f"Run       : {run_idx} ({run_names[run_idx]})")
        print(f"Pose Space: {pose_space}")
