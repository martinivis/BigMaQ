...
class ActionLoader():
    ...
    def fill_action_indices_by_ind(self):
        ...
        with open(join(r"../../data/Individuals", f"actions_overview_by_ind.json"), 'w') as f:
    ...
    def load_bigmac_overview(self):
        ...
        self.dataset["path"] = self.dataset["path"].apply(lambda x:
                                                          convert_windows_path_to_linux(x, self.hard_drive_loc))

        if not self.debug:
            self.fill_action_indices_by_ind()

        calib_paths = []
        self.calib_paths_all = [convert_windows_path_to_linux(x, self.hard_drive_loc) for x in calib_paths]
    ...
    def update_action(self, vid_index):
        ...
        if self.labels_only:
            ...
            self.action_path = join(self.hard_drive_loc_fast, "Action")

            # Individuals, get the list of full names as usual
            self.individuals = self.unique_individuals
    ...
    def return_views_present_yolo(self, yolo_path):
        ...
        available_views = [str(int(x)) for x in os.listdir(yolo_path) if 'npy' not in x]
        available_views_as_file = [x for x in os.listdir(yolo_path) if 'npy' not in x]

        view_dict = {}
        for view_index, view in enumerate(available_views):
            view_dict[view] = available_views_as_file[view_index]

        self.available_videos = view_dict
    def load_YOLO(self):

        # Make the yolo path
        self.yolo_path = join(self.action_path, "YOLO_results")
